  ...
  <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center">
    <div id="modal-content" class="bg-white w-[1100px] max-w-full rounded-xl shadow-2xl p-6 relative print:w-full print:shadow-none print:p-0 h-[500px] overflow-auto">
      ...
      <div class=" overflow-x-auto overflow-y-auto">
        <table v-for="(purchase, i) in props.items[0].yarn_purchase" :key="i" class="w-full border border-gray-300 text-sm mt-4">
          ...
          <tbody>
            <tr   class="hover:bg-gray-50 border-1 print:border-2">
              <td class="px-4 py-2 border text-center">{{ purchase.name }}</td>
              <td class="px-4 py-2 border text-center">{{ purchase.description }}</td>
              <td class="px-4 py-2 border text-center">{{ purchase.unit }}</td>
              <td class="px-4 py-2 border text-center">{{ purchase.bags }}</td>
            </tr>
          </tbody>
